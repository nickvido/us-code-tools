...
- [ ] When cached Congress member data exists but is stale (beyond TTL) or partial (e.g., member snapshot was interrupted by rate-limit exhaustion), the cross-reference still runs against whatever member data IS cached, but the manifest records `cross_reference_status: "partial_congress_cache"` alongside the match/unmatch counts, so downstream consumers know the crosswalk may be incomplete. The legislators fetch itself does NOT fail due to stale/partial Congress cache.
  <!-- Touches: src/sources/unitedstates.ts, tests/sources/unitedstates-crosswalk.test.ts -->
...
19. Start two concurrent fetches targeting the same source/cache path using fixtures. Verify the final manifest remains valid JSON and references only complete artifacts; if one command loses, it fails with the deterministic conflict/runtime error rather than corrupting state.
20. Run `npm test` in default mode. Verify the suite passes without live-network access. Enable the live-test env flag and verify one real call per source is exercised, gated so CI can skip it.
