...
- [ ] `src/utils/fetch-config.ts` exports `getCurrentCongress(): Promise<number>`, which resolves the current congress from `GET https://api.congress.gov/v3/congress/current?api_key=${API_DATA_GOV_KEY}`, caches the resolved value for the lifetime of the process, and supports `CURRENT_CONGRESS_OVERRIDE` for tests. If that upstream lookup fails, it falls back to one versioned hardcoded floor value defined in `src/utils/fetch-config.ts`, logs a warning, and marks the Congress bulk scope as **degraded** rather than silently complete: the JSON result and manifest must record `bulk_scope.congress.resolution="fallback"`, the fallback value used, and a machine-checkable warning/error code indicating operator review is required.
...
- [ ] OLRC corpus selection is deterministic: when the annual-title-files listing exposes multiple annual sets or multiple candidate ZIP links for a title, the client selects the single latest annual vintage published on that listing page, then uses that same vintage consistently for all 54 titles in the run. The selected OLRC vintage identifier (for example the published year label or equivalent listing token) is recorded in the JSON summary and manifest.
  <!-- Touches: src/sources/olrc.ts, tests/sources/olrc.test.ts -->
- [ ] `fetch --source=olrc` caches each downloaded ZIP and its extracted XML payloads under `data/cache/olrc/`, and the manifest records one entry per title with title number, ZIP path, extraction path, byte count, fetch timestamp, and the selected OLRC vintage for that run.
  <!-- Touches: src/sources/olrc.ts, src/utils/manifest.ts, tests/sources/olrc.test.ts -->
- [ ] If the selected latest vintage is missing ZIP links for one or more of the 54 titles, the OLRC source treats the missing titles as failures (same as download failures): the command exits `1`, the JSON summary lists missing title numbers with `error.code="missing_from_vintage"`, and no manifest entry is written for missing titles. The client does NOT fall back to an older vintage for individual titles — all titles must come from the same vintage for consistency.
  <!-- Touches: src/sources/olrc.ts, tests/sources/olrc.test.ts -->
...
- [ ] **Bulk historical contract:** `fetch --all` without `--congress` must include Congress.gov acquisition for every congress integer in the inclusive range `93..getCurrentCongress()`, where `getCurrentCongress()` is the single runtime source of truth from `src/utils/fetch-config.ts`; tests may pin this via `CURRENT_CONGRESS_OVERRIDE`, and the resolved value is emitted in the JSON summary as `bulk_scope.congress.current`. If the runtime value came from the fallback floor instead of the live API, the summary and manifest must additionally record `bulk_scope.congress.resolution="fallback"`, the fallback value used, and a machine-checkable degraded-run indicator.
...
- [ ] The legislators client exposes typed parse results for current legislators, historical legislators, and current committees.
  <!-- Touches: src/sources/unitedstates.ts, tests/sources/unitedstates.test.ts -->
- [ ] Legislators bioguide cross-reference may use **only** the latest successful complete Congress global-member snapshot as its Congress input set. If that qualifying snapshot exists, legislators acquisition performs a deterministic bioguide cross-reference against that snapshot’s member identifiers, writes `data/cache/legislators/bioguide-crosswalk.json`, and records `matched_bioguide_ids`, `unmatched_legislator_bioguide_ids`, and `unmatched_congress_bioguide_ids` counts in the manifest.
  <!-- Touches: src/sources/unitedstates.ts, src/utils/manifest.ts, tests/sources/unitedstates-crosswalk.test.ts -->
- [ ] When no successful complete Congress global-member snapshot exists, legislators acquisition still succeeds and writes no crosswalk file. The manifest must record exactly one machine-readable status describing why cross-reference was skipped: `skipped_missing_congress_cache` when no snapshot exists, `skipped_stale_congress_snapshot` when the latest snapshot exists but is stale because one or more referenced artifacts are expired or missing, or `skipped_incomplete_congress_snapshot` when the latest snapshot did not complete successfully.
...
3. Run `npx us-code-tools fetch --source=olrc` against fixtures whose annual-title-files listing exposes multiple annual vintages for the same titles. Verify the client selects one deterministic vintage — the latest one on the listing page — uses it consistently for all 54 titles, writes 54 ZIPs plus extracted XML trees under `data/cache/olrc/`, and records the selected vintage in the JSON summary and manifest.
...
14. Populate a fresh successful complete Congress global-member snapshot, then run `npx us-code-tools fetch --source=legislators`. Verify `data/cache/legislators/bioguide-crosswalk.json` is written and manifest crosswalk counts match fixture expectations.
15. Replace that Congress snapshot with a stale or incomplete one, then re-run `npx us-code-tools fetch --source=legislators`. Verify no crosswalk file is written and the manifest records `cross_reference_status: "skipped_stale_congress_snapshot"` or `"skipped_incomplete_congress_snapshot"` as appropriate.
...
17. Run bare `npx us-code-tools fetch --all` with fixtures that force `GET /congress/current` to fail. Verify the command uses the single fallback floor from `src/utils/fetch-config.ts`, the Congress bulk JSON result and manifest record `bulk_scope.congress.resolution="fallback"` plus the fallback value and degraded-run indicator, and QA can distinguish this run from a fully resolved live-current-congress run.
18. Run `npx us-code-tools fetch --all --congress=118` with a fixture setup that forces OLRC failure while the other four sources can succeed. Verify the command still invokes `congress`, `govinfo`, `voteview`, and `legislators` in order; stdout includes five top-level source result objects; OLRC is reported as failed; later sources are not reported as skipped solely because OLRC failed; and the overall exit code is `1`.
...
20. Start two concurrent fetches targeting the same source/cache path using fixtures. Verify the final manifest remains valid JSON and references only complete artifacts; if one command loses, it fails with the deterministic conflict/runtime error rather than corrupting state.
21. Run `npm test` in default mode. Verify the suite passes without live-network access. Enable the live-test env flag and verify one real call per source is exercised, gated so CI can skip it.
...
- OLRC listing anomalies: multiple annual vintages on the listing page, multiple candidate ZIP links for one title within a vintage, and missing titles within the selected latest vintage
- Missing `bioguideId` values, malformed GovInfo `packageId` values, partial bill artifact sets, stale-vs-fresh Congress member snapshots, stale/incomplete snapshots presented to the legislators cross-reference step, and empty granule collections
...
- **Pure core:** CLI argument validation, cache-key derivation, congress-range expansion for bulk mode, `getCurrentCongress()` resolution/fallback policy, OLRC latest-vintage selection from listing metadata, GovInfo `packageId` congress parsing, GovInfo resume-state transitions, Congress bulk-progress state transitions, manifest merge/update logic, TTL freshness checks, global-member-snapshot freshness checks, retry scheduling decisions, and rate-budget arithmetic.
- **Properties:** deterministic cache paths from the same request identity; no network on fresh cache hit without `--force`; manifest success entries only reference complete artifacts; Congress/GovInfo limiter never schedules above configured capacity; GovInfo congress filtering is a pure function of `packageId`; OLRC vintage selection is a pure function of the listing contents and always selects one consistent latest vintage per run; if the selected OLRC vintage is incomplete, the run reports only same-vintage missing-title failures and never mixes in older-vintage URLs; non-`--force` GovInfo resume never restarts from page 1 when a valid checkpoint exists; non-`--force` Congress bulk resume never restarts from congress `93` when a valid checkpoint exists; repeated VoteView index lookups do not require reparsing the raw files; repeated Congress per-congress fetches reuse a fresh global member snapshot rather than duplicating global member downloads; legislators cross-reference either uses the latest successful complete Congress global-member snapshot or emits one of the explicit skip statuses and writes no crosswalk file; a `getCurrentCongress()` fallback run is always machine-identifiable as degraded in JSON/manifest output.
