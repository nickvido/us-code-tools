...
- [ ] When `transform --group-by chapter` is used, `writeTitleOutput()` (or a renamed successor) writes `_title.md` plus one markdown file per distinct chapter bucket under `out/uscode/title-{NN}/`. If the bucket chapter identifier is purely numeric ASCII digits, the filename is `chapter-{CCC}.md` with zero-padding to width 3 (for example `1` → `chapter-001.md`, `12` → `chapter-012.md`). Otherwise the filename is `chapter-{safe-id}.md`, where `{safe-id}` is derived by one shared helper with this exact normalization contract: trim leading/trailing Unicode whitespace; lowercase ASCII letters; replace each maximal run of one or more non-ASCII-alphanumeric characters with a single hyphen (`-`); collapse repeated hyphens; trim leading/trailing hyphens; if the result would be empty, use the literal `unnamed`. Example mappings that tests must assert: `IV` → `chapter-iv.md`, ` Subchapter A ` → `chapter-subchapter-a.md`, `A-1 / Special` → `chapter-a-1-special.md`, and `***` → `chapter-unnamed.md`. <!-- Touches: src/transforms/write-output.ts, src/domain/normalize.ts or new helper module, tests/unit/transforms/write-output.test.ts -->
...
- [ ] Every generated chapter file begins with frontmatter containing exactly these required keys when the chapter is categorized: `title`, `chapter`, `heading`, `section_count`, and `source`. `title` equals the numeric title, `chapter` equals the parsed chapter identifier string, `heading` equals the matching chapter heading from `TitleIR.chapters` when available and otherwise falls back to the exact literal `Chapter {chapter}` using the unmodified parsed chapter identifier, `section_count` equals the number of rendered sections in that file, and `source` equals `titleIr.sourceUrlTemplate`. Example: if `section.hierarchy.chapter === "IV"` and `TitleIR.chapters` has no matching heading entry, frontmatter must contain `heading: Chapter IV`. <!-- Touches: src/transforms/markdown.ts or new chapter renderer, src/transforms/write-output.ts, tests/unit/transforms/markdown.test.ts -->
...
6. Run a unit test over non-numeric chapter identifiers and verify filename normalization matches the spec examples exactly: `IV` → `chapter-iv.md`, ` Subchapter A ` → `chapter-subchapter-a.md`, `A-1 / Special` → `chapter-a-1-special.md`, and `***` → `chapter-unnamed.md`.
7. Run a fixture where `TitleIR.chapters` omits a heading for a present chapter identifier and verify the generated frontmatter falls back to `heading: Chapter {chapter}` exactly.
8. Verify the chapter file contains the expected section headings in canonical order: `# § ...` for each member section.
9. For one sampled section, compare the standalone `renderSectionMarkdown(section)` output to the corresponding slice inside the chapter file and verify the section body text, notes, source credit, and links are identical.
10. Transform a fixture with at least one section lacking `hierarchy.chapter` and verify `_uncategorized.md` is written, includes that section, and the JSON report includes one entry in `warnings` per uncategorized section while `parse_errors` remains unchanged.
11. Run the full title matrix in chapter mode and verify titles `1..52` and `54` succeed, title `53` keeps its current reserved-empty failure path, and successful titles report zero `parse_errors` even when `warnings` are present.
12. Compare file counts for the same title fixture in default mode vs chapter mode and verify chapter mode writes fewer files whenever multiple sections share a chapter.
...
- **Delimiter edge cases:** chapter identifiers that include punctuation, spaces, roman numerals, or mixed case must map to deterministic file-safe stems through one shared helper using the exact contract in Acceptance Criterion 2 (`IV` → `iv`, ` Subchapter A ` → `subchapter-a`, `A-1 / Special` → `a-1-special`, `***` → `unnamed`).
- **Encoding issues:** chapter headings containing em dashes, quotes, non-breaking spaces, or non-ASCII characters render correctly in frontmatter/body and do not corrupt filenames; filename normalization still applies only the specified ASCII-safe `safe-id` contract.
...
- **Missing chapter metadata:** if `TitleIR.chapters` lacks a heading entry for a chapter identifier that exists on sections, the file is still written with fallback frontmatter `heading: Chapter {chapter}` using the original parsed identifier (for example missing `IV` heading → `heading: Chapter IV`); it must not crash or drop the bucket.
