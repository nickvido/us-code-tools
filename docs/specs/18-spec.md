...
10. Inspect one non-baseline rendered release body and verify it contains `## Diff Stat`, `## Summary`, `## Notable Laws`, and `## Narrative` in that order, that the diff section reflects `git diff --stat congress/<prev>..congress/<current>`, and that all three non-diff sections come from the single `annual_snapshots[]` row for that Congress where `is_congress_boundary: true`.
11. Modify metadata so one Congress has zero boundary rows or multiple boundary rows; rerun `release` and verify it exits non-zero before any GitHub write for that Congress.
12. Inspect the earliest in-scope Congress release and verify `previous_tag` is `null` in plan output and the release body contains `Baseline release: no prior congress tag in scope.` instead of a git diff stat.
13. Re-run `milestones release` unchanged and verify the same release records are updated in place rather than duplicated.
14. Run `npm test` and `npm run build` and verify both exit `0`.
