...
async function ensureMemberSnapshot(args:{ force:boolean; manifest:FetchManifest; sourceDirectory:string }): Promise<{ ok:true; counts:{ member_pages:number; member_details:number } } | { ok:false; rate_limit_exhausted:boolean; next_request_at:string|null; error:{ code:string; message:string } }> {
  ...
  while (pageUrl !== null) {
    pageIndex += 1; const membersPage: { payload: CongressMemberListPayload } = await fetchCongressJson<CongressMemberListPayload>(pageUrl); const artifactRelative = resolve('members', 'snapshots', snapshotId, 'pages', `page-${pageIndex}.json`); const artifactAbsolute = resolve(pagesDirectory, `page-${pageIndex}.json`);
...
async function fetchSingleCongress(args:{ congress:number; sourceDirectory:string }): Promise<{ bill_pages:number; bill_details:number; bill_actions:number; bill_cosponsors:number; committee_pages:number }> {
  ...
  while (committeePageUrl !== null) { committeePageCount += 1; const page = await fetchCongressText(committeePageUrl); await writeFile(resolve(committeesDirectory, `page-${committeePageCount}.json`), page.body, { encoding:'utf8', mode:0o640 }); const parsed = JSON.parse(page.body) as { pagination?: { next?: string | null } }; committeePageUrl = typeof parsed.pagination?.next === 'string' && parsed.pagination.next.length > 0 ? appendApiKey(parsed.pagination.next, apiKey) : null; }
