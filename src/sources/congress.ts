...
const CONGRESS_CACHE_TTL_MS = 24 * 60 * 60 * 1000; const DOWNLOAD_TIMEOUT_MS = 500; 
...
async function fetchCongressResponse(url: string, options: { force: boolean }): Promise<{ body: string }> { const startedAt = Date.now(); if (!options.force) { const cached = await readFreshRawResponseCache('congress', url, CONGRESS_CACHE_TTL_MS); if (cached !== null) { logNetworkEvent({ level:'info', event:'network.request', source:'congress', method:'GET', url, attempt:1, cache_status:'hit', duration_ms:Date.now() - startedAt, status_code:cached.metadata.status_code }); return { body: cached.body }; } }
  const exhaustion = isRateLimitExhausted(getSharedApiDataGovLimiter()); if (exhaustion.exhausted) throw createRateLimitError(exhaustion.nextRequestAt); markRateLimitUse(getSharedApiDataGovLimiter()); const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), DOWNLOAD_TIMEOUT_MS); try { const response = await fetch(url, { signal: controller.signal }); const body = await response.text(); logNetworkEvent({ level:'info', event:'network.request', source:'congress', method:'GET', url, attempt:1, cache_status: options.force ? 'bypass' : 'miss', duration_ms:Date.now() - startedAt, status_code:response.status }); if (response.status === 401 || response.status === 403) throw new Error('upstream_auth_rejected: Congress.gov rejected API_DATA_GOV_KEY'); if (response.status === 429) { const retryAt = parseRetryAfter(response.headers.get('retry-after')); throw Object.assign(new Error('rate_limit_exhausted: Congress.gov returned 429'), { code: 'rate_limit_exhausted', nextRequestAt: retryAt ? new Date(retryAt).toISOString() : null }); } if (!response.ok) throw new Error(`upstream_request_failed: Congress.gov request failed with HTTP ${response.status}`); await writeRawResponseCache('congress', url, body, { statusCode: response.status, contentType: response.headers.get('content-type') }); return { body }; } catch (error) { if (!(error instanceof Error && error.message.startsWith('upstream_auth_rejected:')) && !(error instanceof Error && error.message.startsWith('upstream_request_failed:'))) { logNetworkEvent({ level:'error', event:'network.request', source:'congress', method:'GET', url, attempt:1, cache_status: options.force ? 'bypass' : 'miss', duration_ms:Date.now() - startedAt }); } throw error; } finally { clearTimeout(timeout); } }
