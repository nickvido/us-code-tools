...
- Focused issue #20 tests:
  - `npx vitest run tests/unit/domain/normalize-title-directory.test.ts tests/unit/transforms/write-output.test.ts tests/unit/transforms/markdown.test.ts`
  - `npx vitest run tests/unit/transforms/issue12-recursive-metadata.test.ts tests/integration/issue12-transform-cli.test.ts tests/integration/transform-cli.test.ts`
  - spot-checks: slug contract + fallback, slugged section/chapter output roots, numeric-title matrix coverage, and real parser-path cross-title links to titles 18/42
...
- `src/index.ts` → `src/commands/fetch.ts`, `src/backfill/orchestrator.ts`, `src/sources/olrc.ts`, `src/transforms/uslm-to-ir.ts`, `src/transforms/write-output.ts` (issue #8/#16/#20: `transform` now resolves OLRC ZIPs via manifest-backed selected-vintage cache state instead of the legacy `.cache/olrc/title-XX` layout, parses `--group-by chapter`, surfaces additive `warnings`, returns non-zero on any `OUTPUT_WRITE_FAILED` from chapter-mode writes, and ultimately writes into slugged `title-{NN}-{heading}` directories rather than bare `title-{NN}` when heading data exists)
...
- `src/transforms/uslm-to-ir.ts` → `src/domain/model.ts`, `src/domain/normalize.ts`, `fast-xml-parser` (issue #31: this is now also the note-structure preservation boundary — `parseNotesOrdered(...)`, `readOrderedMixedNoteText(...)`, ordered table rendering, and note-scope-aware top-level section discovery must stay aligned with markdown expectations) (issue #8/#10/#12/#14/#20/#29: parser config uses `removeNSPrefix: true`, root discovery falls back from `uscDoc.main.title` to `uslm.title`, canonical title/chapter/section numbers flow through `readCanonicalNumText(...)`, recursive traversal accumulates `subtitle`/`part`/`subpart`/`chapter`/`subchapter`, `parseNotes()` emits `sourceCredit` + `statutoryNotes`, ordered-body helpers preserve `chapeau`, inline body text, nested descendants, and `continuation` across `subsection -> ... -> subitem`, cross-title USC hrefs derive slugged destination title directories through `titleDirectoryName(...)` + `resolveKnownTitleHeading(...)`, ordered-path xrefs preserve canonical refs in `#ref=${encodeURIComponent(`${title}:${section}`)}`, and `readSectionHeading(...)` is now the shared heading-parity seam for ordered vs non-ordered parsing)
- `src/transforms/markdown.ts` → `src/domain/model.ts`, `src/domain/normalize.ts`, `gray-matter` (issue #31: renderer now depends on `embeddedSectionAnchor(...)`, `buildCanonicalSectionUrl(...)`, preserved note paragraph/table text, and explicit blank-line separation before structured subsection siblings; do not reintroduce visible `{#section-*}` suffixes or paragraph-collapsing joins) (issue #12/#14/#16/#20/#29: section frontmatter serializes hierarchy keys + `source_credit`, `_title.md` no longer renders a per-section list, section prose may contain relative USC links generated from sanitized section ids, labeled body rendering normalizes bare labels like `1` to `(1)` while keeping deterministic indentation, chapter-mode rendering now uses explicit contextual heading levels plus standalone `<a id="section-*"></a>` anchor lines, and chapter-mode xref rewriting depends on `embeddedSectionAnchor(...)`, `buildCanonicalSectionUrl(...)`, and `sectionTargetsByRef` rather than local `section-*.md` paths) (issue #36: nested labeled descendants are now a special renderer path — read `hasLabeledDescendant(...)`, `renderContentNodeLines(...)`, `renderStructuredLine(...)`, `renderGithubSafeLabeledParagraph(...)`, and `shouldSeparateStructuredChildren(...)` before changing any structured-body formatting, because top-level subsections stay inline while deeper labeled descendants/continuations switch to bold paragraph blocks at column 0)
...
- `slugifyTitleHeading(...)`, `resolveKnownTitleHeading(...)`, `titleDirectoryName(...)`, `splitSectionNumber(...)`, `compareSectionNumbers(...)`, `sectionFileSafeId(...)`, `sortSections(...)`, `chapterFileSafeId(...)`, `chapterOutputFilename(...)`, and `compareChapterIdentifiers(...)` in `src/domain/normalize.ts` — canonical title/section/chapter directory, sort, and filename contract for issues #12, #16, and #20
...
- Issue #12 transform conventions:
  ...
  - distinguish parser/IR metadata from visible standalone markdown output: chapter-mode recovery still depends on canonical slash-bearing refs surviving through parser/IR `#ref=` metadata, but standalone-rendered relative USC links must strip any visible `#ref=` fragment and remain plain relative links
...
- issue #31 production seams to preserve:
  - `src/domain/normalize.ts` is the only approved place to build canonical OLRC section URLs; parser frontmatter, renderer fallbacks, and tests should all call `buildCanonicalSectionUrl(...)`
  - `src/transforms/markdown.ts` owns GitHub-safe embedded section anchors: exact `<a id="section-*"></a>` line + following heading line; raw HTML output is intentionally constrained to anchors only
  - subsection/chapeau paragraph separation is a renderer contract; blank lines belong in `renderContentNodes(...)` / `shouldSeparateWithBlankLine(...)`, not in ad hoc fixture text massaging
  - note tables are preserved as markdown table text emitted from parser-preserved structure; escape table cell content instead of bypassing markdown formatting with arbitrary HTML
  - note-scoped embedded Act sections are not top-level codified sections; if you touch section discovery, keep note ancestry checks intact
- issue #36 production seams to preserve:
  - GitHub-safe nested rendering is intentionally renderer-only; do not move issue #36 behavior into `src/transforms/uslm-to-ir.ts` or add new IR node kinds for it
  - the compatibility gate is narrow: only subsection subtrees with deeper labeled descendants activate the new path, so flat sections and top-level-only subsection output must remain byte-stable
  - continuation/body lines inside that affected nested hierarchy are part of the same safety contract as labels; if you touch `text`-node indentation, keep `safeIndent = 0` in the GitHub-safe nested path so lines never begin with four or more literal spaces
  - the blank-line contract for nested labeled blocks lives in `shouldSeparateWithBlankLine(...)` / `shouldSeparateStructuredChildren(...)`; keep exact single blank-line boundaries instead of adding join-time regex cleanup
  - if you need to reason about behavior changes, compare `renderLabeledLine(...)` (legacy depth-indented path) with `renderGithubSafeLabeledParagraph(...)` (issue #36 path) before editing snapshots/tests
