...
- Branch commit `07b954e` already fixed the two earlier issue #12 adversary seams: slash-separated `/us/usc/t10/s125/d` refs now canonicalize to `../title-10/section-00125d.md`, and equal-root mixed-case suffix ordering is explicit `106 < 106A < 106a < 106b` via a locale-independent tie-breaker.
- The current remaining issue #12 blocker is still inside `src/transforms/uslm-to-ir.ts::readRawText()`: mixed-content XML (`#text` + inline `<ref>` / `<date>`) is reconstructed bucket-first instead of source-order-first, so rendered `source_credit` / statutory-note text can still drop or reorder normalized text like `Aug. 10, 1956, ch. 1041` and `70A Stat. 3`.
- At current head (`3b5fb20`), that bucketed reconstruction is still literal code: `readRawText()` first concatenates `node['#text']`, `text`, `p`, `content`, `heading`, `num`, `chapeau`, `continuation`, `quotedContent`, and `inline`, then does a second `Object.entries(node)` pass for the remaining children. Any future fix has to replace both passes with one deterministic document-order walker rather than adding more special cases.
