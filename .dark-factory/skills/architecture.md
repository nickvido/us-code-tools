...
- issue #31 follows directly on that same parser/renderer seam:
  - `src/domain/normalize.ts` is now the single canonical boundary for working OLRC section URLs, and the required contract is `https://uscode.house.gov/view.xhtml?req=granuleid:USC-prelim-title{title}-section{section}&num=0&edition=prelim`
  - `src/transforms/markdown.ts` emits GitHub-safe embedded anchors as a standalone raw HTML line (`<a id="section-*"></a>`) above each embedded section heading instead of a visible `{#section-*}` suffix, and inserts blank lines between structured subsection siblings so GitHub does not collapse them into one paragraph
  - `src/transforms/uslm-to-ir.ts` preserves note paragraph/table structure through the ordered parser path, renders note tables as markdown-safe table text, and excludes note-scoped embedded Act sections from top-level `titleIr.sections`
  - note rendering remains markdown-first: paragraph breaks survive as `\n\n`, table cells are escaped for markdown output, and arbitrary HTML passthrough is still out of scope
- issue #36 is a renderer-only follow-up on that same seam:
  - `src/transforms/markdown.ts` now gates a GitHub-safe nested rendering mode with `hasLabeledDescendant(...)` and the per-node `inGithubSafeNestedHierarchy` context flag instead of relying on depth-based indentation once a top-level subsection contains deeper labeled descendants
  - in that affected path, `renderStructuredLine(...)` routes descendants through `renderGithubSafeLabeledParagraph(...)`, so labeled `paragraph` / `subparagraph` / `clause` / `subclause` / `item` / `subitem` lines render at column 0 as bold paragraph labels (`**(A)**`, `**(i)**`, `**(I)**`) with exact blank-line separation
  - continuation/body `text` nodes inside the same nested hierarchy also render with zero indentation (`safeIndent = 0`), so standalone continuation lines stay below GitHub’s 4-space code-block threshold while preserving parent-before-child source order
  - flat sections and top-level-subsection-only sections remain on the existing byte-stable path because GitHub-safe nested rendering only activates when a subsection subtree actually has labeled descendants below that first rendered level
  - `src/transforms/uslm-to-ir.ts` is intentionally unchanged for issue #36; the existing IR node kinds and labels remain the only parser contract the renderer consumes
