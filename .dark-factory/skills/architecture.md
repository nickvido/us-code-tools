...
- Current issue #29 branch head `67d7f86` completes the chapter-rendering correctness pass: `embeddedSectionAnchor(...)` produces deterministic anchors (`125/d` -> `section-125-d`), chapter-mode xrefs resolve through writer-built `sectionTargetsByRef` entries keyed as `${title}:${section}`, `_title.md` is chapter-list-only, and parse-output ordered xrefs preserve canonical slash-bearing refs through `#ref=${encodeURIComponent(`${title}:${section}`)}` fragments so renderer fallback URLs keep `125/d` exact.
...
- What's implemented:
  ...
  - issue #29 chapter-rendering correctness layer:
    ...
    - real parse-output markdown links preserve canonical slash-bearing refs through `#ref=` fragments so `[section 125/d](../title-05-.../section-00125d.md)` can map through key `5:125/d` or fall back to exact `...section125/d`
